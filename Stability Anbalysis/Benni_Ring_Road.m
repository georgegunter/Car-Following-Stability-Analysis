...
%% Model parameters
%------------------------------------------------------------------------
L = 205.9363; % length of circular road
...
tf = 500; % final time of trajectory computation
...
c_control = 0; % strength of feedback control, gain
...
% NOTE: this model was changed in the relative dist portion away from
% inversely proportional to the square of the spacing, to just inversely
% proportional.
...
% v = p*0; %ADDED TO TEST VELOCITY IMPORTANCE
...
%     options = odeset('Events', @endEvent); % set the options with the myEvent function
    if t_sub<t_av_on % before the AV is switched on, use the un-controlled evolution
        [~,Y_c_sub] = ode45(R,t_sub,Y_c(end,:));
    else
        [~,Y_c_sub] = ode45(R_c,t_sub,Y_c(end,:)); % compute veh. trajectories
    end
